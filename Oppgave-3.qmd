...
model_results %>%
  gt() %>%
  tab_header(
    title = "Oppsummering av modellresultater",
    subtitle = "Estimat, standard feil, t-verdi og p-verdi for M1 og M2"
  ) %>%
  fmt_number(
...
results_summary %>%
  gt() %>%
  tab_header(
    title = "Oppsummering av Resultater for Estimater og Standardfeil",
    subtitle = "Gjennomsnitt og standardavvik for estimater og gjennomsnitt standardfeil"
  ) %>%
  fmt_number(
...
significant_results %>%
  gt() %>%
  tab_header(
    title = "Andel Signifikante Resultater",
    subtitle = "Tallverdi av tester med p-valg under 0.05 for hver utvalgsstørrelse"
  ) %>%
  fmt_number(
    columns = sig_results,
...
false_positives %>%
  gt() %>%
  tab_header(
    title = "Andel falske positive resultat",
    subtitle = "Tallverdi av tester med falske positive resultat"
  ) %>%
  fmt_number(
    columns = sig_results,
    decimals = 3  # Formaterer til tre desimaler for bedre lesbarhet
